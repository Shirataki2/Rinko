...
class Command(commands.Cog):
    ...
    async def run(self, ctx: commands.Context, *, code):
        ...
        **8.** With great power comes great responsibility.
        ...
        if sources := re.findall(r'```(.+?)\n(.*?)```', code, re.RegexFlag.DOTALL):
            # stdin check
            os.makedirs('/root/discord-dev/run/src', exist_ok=True)
            os.makedirs('/root/discord-dev/run/images', exist_ok=True)
            os.makedirs('/root/discord-dev/run/media', exist_ok=True)
            try:
                if sources2 := re.findall(r'```(.+?)\n(.*?)```.*?```\n(.*?)```', code, re.RegexFlag.DOTALL):
                    lang, source, stdin = sources2[0]
                    async with aiofiles.open(f'run/src/stdin', 'w') as f:
            ...
            if attach := ctx.message.attachments:
                ...
                try:
                    ...
                    i.save('/root/discord-dev/run/media/0.png')
                    os.rename('/root/discord-dev/run/media/0.png', '/root/discord-dev/run/media/0')
            ...
            images.extend(glob.glob('/root/discord-dev/run/images/**.jpg'))
            images.extend(glob.glob('/root/discord-dev/run/images/**.gif'))
            images.extend(glob.glob('/root/discord-dev/run/images/**.jpeg'))
            if images:
    ...
    async def get_docker_cmd(self, lang, source):
        ...
        if lang in ['cpp', 'c++']:
            ...
            logger.info('Run C Code')
            async with aiofiles.open(f'run/src/Main.c', 'w') as f:
                await f.write(source)
            ...
            async with aiofiles.open(f'run/src/Main.py', 'w') as f:
                await f.write(source)
            ...
            async with aiofiles.open(f'run/src/Main.js', 'w') as f:
                await f.write(source)
            ...
            async with aiofiles.open(f'run/src/Main.rb', 'w') as f:
                await f.write(source)
            ...
            async with aiofiles.open(f'run/src/Main.hs', 'w') as f:
                await f.write(source)
            ...
            async with aiofiles.open(f'run/src/Main.rs', 'w') as f:
                await f.write(source)
            ...
            async with aiofiles.open(f'run/src/Main.{ext}', 'w') as f:
                await f.write(source)
            ...
            async with aiofiles.open(f'run/src/Main.java', 'w') as f:
                await f.write(source)
            ...
            async with aiofiles.open(f'run/src/Main.go', 'w') as f:
                await f.write(source)
            ...
            async with aiofiles.open(f'run/src/Main.php', 'w') as f:
                await f.write(source)
